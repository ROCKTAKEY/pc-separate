...
(defcustom separate-system-alist '()
...
(defun separate--system-to-system-name (system)
  "Return system-name which SYSTEM is representing."
  (let
      ((sys-name (if (stringp system)
                     system
                   ;; if no system-name, return nil
                   (car (rassq system separate-system-alist)))))
    (when sys-name
      sys-name)))

(defun separate--system-name-to-system (sys-name)
  "Return list whose all element is representing SYS-NAME."
  (let* ((p (separate--cdr-assoc-all sys-name separate-system-alist))
         (system-list (if p
                          (append p (list sys-name))
                        (list sys-name))))
    system-list))

(defun separate--current-system-p (system)
  "Return non-nil if SYSTEM is representing current system."
  (string= (separate--system-to-system-name system) (system-name)))
