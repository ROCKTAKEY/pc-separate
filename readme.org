...
* How to Use?
  ...
  :CUSTOM_ID: HowToUse
  :END:
  Set your definition of system-separator into =system-separate-system-separator-alist= (optional),
  and use =system-separate-setq= or =system-separate-cond=.
  See also [[#System-Separator][System-Separator Section]] and [[#Keywords][Keywords Section]].
#+BEGIN_SRC emacs-lisp -n
  ;; Define some system-separator. This is optional.
  ;; If you define this, you can use symbol as system-separator(like condicate).
  (setq system-separate-system-separator-alist
        '(;; (symbol . system-separator)
  ...
  (system-separate-setq foo
    (;; (system-separator . value)
     ...
     ((:system-separators "MY-PC-NAME4" gnu/linux) . 8)
     (default . 0)))
#+END_SRC
  ...
  8. Else if your system-name is "MY-PC-NAME4" OR your =system-type=
    is =gnu/linux=: 8
  9. If all of =system-separators= are invalid: 0
* System-Separator
  :PROPERTIES:
  :CUSTOM_ID: System-Separator
  :END:
  - =system-separator= means object which can separate system environment.
  - =system-separator= can be list, symbol, number or string.
** List
   - There are 2 types of listed-system-separator.
   - One type is:
     - Have keyword in the =car=. All keywords start ":".
     - =cdr= of the list is argument. See [[#Keywords][Keywords Section]].
   - The other type is:
     - Don't have keyword in the =car=. This is put =:system-separators= keyword
       automatically. See [[#system-separators][system-separators Section]].
** Symbol
   - This is interpreted as =system-separator= defined in =system-separate-system-separator-alist=,
   or system-type (same as =:os= keyword). See [[#system-separate-system-separator-alist][system-separate-system-separator-alist Section]].
   - the symbol =default= is special symbol. Element including this is evaluated
     only when all of other =seprators= are invalid.
#+BEGIN_SRC emacs-lisp -n
  ;;Define symbol system-separator.
  (setq system-separate-system-separator-alist
   '(
     ;; This system-separator means "if `system-name' is "my-windows",".
     ;; See also String Section and :system-name Section.
     (my-win   . "my-windows")

     ;; This system-separator means "if `system-type' is `gnu/linux' and
...
** =:system-name=
   ...
   - This keyword accept multiple argument of =string=. if at least one of
     argument is same as system-name, this =system-separator= is valid.
...
** =:emacs-version>==
   ...
   - This keyword accept 1 or 2 argument(s) of number. First argument is =M=,
     ...
     than =M.m=, this =system-separator= is valid.
   - If you use only first argument, you can write number of major-version as system-separator.
#+BEGIN_SRC emacs-lisp -n
  (system-separate-setq foo
    (
...
** =:eval=
   - This keyword accept multiple arguments of S expression. If returned value
     of those S expression is =non-nil=, this =system-separator= is valid.
#+BEGIN_SRC emacs-lisp -n
  (system-separate-setq foo
     (
...
** =:os=
   - This keyword accept multiple arguments of symbol. If one of arguments is
     same as =system-type=, this =system-separator= is valid. See also emacs help of
     =system-type=, and [[#system-separators][system-separators Section]].
...
#+BEGIN_SRC emacs-lisp -n
  ...
  (
   ...
   ;; their symbols of `system-type' can be used as symbol system-separator defined in
   ;; `system-separate-system-separator-alist'. So you can write this instead of above.
   ;; See also :system-separators Seciton.
   ((gnu/linux cygwin) . 2)
  ))
#+END_SRC
** =:package-available=
   - This keyword accept multiple argument of feature (symbol). If all arguments
     are returned =non-nil= when passed to =featurep=, this =system-separator=
     is valid.
#+BEGIN_SRC emacs-lisp -n
  (system-separate-setq foo
   (
...
#+END_SRC
** =:system-separators= / =:alias= / =:or=
   :PROPERTIES:
   :CUSTOM_ID: system-separators
   :END:
   - These keywords accept multiple arguments of =system-separator=. If at least one
     of =system-separators= of arguments is valid, this =system-separator= is valid.
...
#+BEGIN_SRC emacs-lisp -n
  (system-separate-setq foo
    ...
    ((:system-separators                         ; This can be `:or' or `:alias'
      (:system-name "MY-PC-1")
      "MY-PC-2"
      windows-nt)
      . 1)

    ;; You can use this keyword with all system-separator.
...
#+END_SRC
** =:and=
   - These keywords accept multiple arguments of =system-separator=. If all
     =system-separators= of arguments are valid, this =system-separator= is valid.
#+BEGIN_SRC emacs-lisp -n
  (system-separate-setq foo
      (;; (system-separator . value)
...
** =system-separate-system-separator-alist=
   :PROPERTIES:
   :CUSTOM_ID: system-separate-system-separator-alist
   :END:
   - An associated list. Each element is cons cell,
     =(symbol . system-separator)=. In this package, you can use the =symbol=
     as the =system-separator=.
   - See also [[#HowToUse][How to Use Section]] as example.
   - In the future, we will provide some functions to define =symbol-system-separator=
     like this.
* Macros
